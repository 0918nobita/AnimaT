...
type Expr =
    ...
    override this.ToString() =
        ...
        | AndE(lhs, rhs) when lhs.IsSimple && rhs.IsSimple ->
            sprintf "%O ∧ %O" lhs rhs
        | AndE(lhs, rhs) when lhs.IsSimple ->
            sprintf "%O ∧ (%O)" lhs rhs
        | AndE(lhs, rhs) when rhs.IsSimple ->
            sprintf "(%O) ∧ %O" lhs rhs
        | AndE(lhs, rhs) ->
            sprintf "(%O) ∧ (%O)" lhs rhs

        | OrE(lhs, rhs) when lhs.IsSimple && rhs.IsSimple ->
            sprintf "%O ∨ %O" lhs rhs
        | OrE(lhs, rhs) when lhs.IsSimple ->
            sprintf "%O ∨ (%O)" lhs rhs
        | OrE(lhs, rhs) when rhs.IsSimple ->
            sprintf "(%O) ∨ %O" lhs rhs
        | OrE(lhs, rhs) ->
            sprintf "(%O) ∨ (%O)" lhs rhs
